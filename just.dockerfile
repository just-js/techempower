...
RUN curl -L -o 0.0.4.tar.gz -L https://github.com/just-js/just/archive/0.0.4.tar.gz
RUN tar -zxvf 0.0.4.tar.gz
WORKDIR /just-0.0.4
RUN make runtime
RUN curl -L -o modules.tar.gz https://github.com/just-js/modules/archive/0.0.4.tar.gz
RUN tar -zxvf modules.tar.gz
RUN mv modules-0.0.4 modules
...
COPY --from=builder /just-0.0.4/just /bin/just
COPY --from=builder /just-0.0.4/modules/picohttp/http.so ./
COPY --from=builder /just-0.0.4/modules/html/html.so ./
